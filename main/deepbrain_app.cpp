...
#elif KMT_PCBA
static duer::YTGpadcKey s_button4(/*KEY_B5*/KEY_B0);// long (wechat)			short (play wechat)			4∫≈º¸
static duer::YTGpadcKey s_button3(/*KEY_B4*/KEY_B2);// long (action) 		short (volume ctl)    			3∫≈º¸
static duer::YTGpadcKey s_button2(/*KEY_B3*/KEY_B1);// long (model)			short (next/pause)				2∫≈º¸
static duer::YTGpadcKey s_button1(/*KEY_B0*/KEY_B3);// long (awake)			short (net)					1∫≈º¸
#elif BDW_PCBA
static duer::YTGpadcKey s_wchat_button(KEY_B1);// Œ¢¡ƒ
static duer::YTGpadcKey s_magic_button(KEY_B0);// ƒß“Ù + ¥Ú∂œªΩ–—
static duer::YTGpadcKey s_play_pause_button(KEY_B2); // ≤•∑≈‘›Õ£+≈‰Õ¯
#endif
...
void entry_new_mode(int new_mode,bool need_prompt)
{
	...
	switch(new_mode)
	{
		...
		{
		// ±æµÿœ¬±£¡Ù ƒ£ Ω«–ªª/œ¬“ª ◊ µÁª˙øÿ÷∆/“Ù¡øµ˜Ω⁄
			s_button1.fall(NULL);
			s_button1.rise(NULL);
			s_button1.longpress(NULL,0,-1);	
			s_button2.fall(&btn2_fall_handle);
