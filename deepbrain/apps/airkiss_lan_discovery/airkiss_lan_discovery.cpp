...
static rtos::Thread *g_thAirkisslandiscovery = NULL;
...
{		
	...
	task_thread_exit();
	DEBUG_LOGI(LOG_TAG, "task_airkiss_lan_discovery app_exit");
}
...
APP_FRAMEWORK_ERRNO_t airkiss_lan_discovery_create(const int task_priority)
{
	...
	//创建线程
    if (! (g_thAirkisslandiscovery =(rtos::Thread *) task_thread_create(task_airkiss_lan_discovery,
...
APP_FRAMEWORK_ERRNO_t airkiss_lan_discovery_delete(void)
{
	...
	app_send_message(APP_NAME_AIRKISS_LAN_DISCOVERY, APP_NAME_AIRKISS_LAN_DISCOVERY, APP_EVENT_DEFAULT_EXIT, NULL, 0);
	while(g_thAirkisslandiscovery->get_state() != rtos::Thread::Deleted){rtos::Thread::wait(10);}
	task_thread_exit(g_thAirkisslandiscovery);
