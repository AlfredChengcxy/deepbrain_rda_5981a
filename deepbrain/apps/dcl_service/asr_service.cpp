...
static rtos::Thread *g_thAsr = NULL;
...
void task_asr_service()
{
	...
	DEBUG_LOGI(LOG_TAG, "task_asr_service app_exit");
}
...
APP_FRAMEWORK_ERRNO_t asr_service_create(int task_priority)
{
	...
	//创建线程
    if (! (g_thAsr = (rtos::Thread *)task_thread_create(task_asr_service,
            "task_asr_service",
...
APP_FRAMEWORK_ERRNO_t asr_service_delete(void)
{
	...
	app_send_message(APP_NAME_ASR_SERVICE, APP_NAME_ASR_SERVICE, APP_EVENT_DEFAULT_EXIT, NULL, 0);
	while(g_thAsr->get_state() != rtos::Thread::Deleted){rtos::Thread::wait(10);}
	task_thread_exit(g_thAsr);
}
...
APP_FRAMEWORK_ERRNO_t asr_service_new_asr_object(ASR_PCM_OBJECT_t **pcm_obj)
{
	...
	DEBUG_LOGE(LOG_TAG, "asr_service_new_asr_object");	
...
{
	DEBUG_LOGE(LOG_TAG, "asr_service_del_asr_object");
